...
/// Returns true if the given status has a condition with the given type and reason.
fn get_condition(
    status: Option<&TrustedExecutionClusterStatus>,
    type_: &str,
    reason: &str,
) -> bool {
    status
...
/// Data shared by every reconcile-phase handler.
struct ReconcileState<'a> {
    ctx: &'a Arc<OperatorContext>,
    clusters: &'a Api<TrustedExecutionCluster>,
    name: &'a str,
    generation: Option<i64>,
    existing_status: &'a Option<TrustedExecutionClusterStatus>,
}
...
async fn reconcile(
    ...
    if cluster.metadata.deletion_timestamp.is_some() {
        return Ok(handle_deletion(&state, &mut conditions).await?);
    }

    if ctx.tec_store.state().len() > 1 {
        return Ok(handle_non_unique(&state, &mut conditions).await?);
    }

    if let Some(action) = handle_upgrade(&state, &cluster, &mut conditions).await? {
        return Ok(action);
    }

    Ok(handle_fresh_install(&state, &kube_client, &cluster, &mut conditions).await?)
}

async fn handle_deletion(
    state: &ReconcileState<'_>,
    conditions: &mut Option<Vec<Condition>>,
) -> Result<Action> {
    // ignore ctx and kube_client as they are not used in this function
    let ReconcileState {
        clusters,
        name,
        generation,
...
/// Returns `None` when the cluster is not installed yet, and reconcile falls through to the fresh-install path.
async fn handle_upgrade(
    ...
    if let Err(e) = upgrade_result {
        ...
        let changed = upsert_condition(conditions, failed);
        if changed {
            update_status!(
                clusters,
                name,
        ...
        // Manual intervention required, upgrade will NOT retry on next reconcile.
        return Ok(Some(LONG_REQUEUE));
    }
...
/// Performs a fresh install of the operator-managed components for a newly created cluster.
