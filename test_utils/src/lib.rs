...
pub async fn get_cluster_url(
    ...
    let check = |v: String| v != "openshift";
    if env::var(PLATFORM_ENV).map(check).unwrap_or(true) {
        return Ok(format!("{namespace}.svc.cluster.local:{port}"));
    }
    let routes: Api<Route> = Api::namespaced(client.clone(), namespace);
    match routes.get(service).await {
        Ok(route) => Ok(route.spec.host.unwrap()),
        Err(_) => {
...
impl TestContext {
    ...
    async fn apply_operator_manifests(&self) -> Result<()> {
        ...
        let platform = env::var(PLATFORM_ENV).unwrap_or_else(|_| "kind".to_string());
        ...
        self.apply_operator_manifest(manifests_path, &platform)
            .await
    }

    async fn apply_operator_manifest(&self, manifests_path: &Path, platform: &str) -> Result<()> {
        let ns = self.test_namespace.clone();
        let trustee_addr =
        ...
        if platform == "openshift" {
            ...
            for svc in ["kbs-service", "attestation-key-register", "register-server"] {
                let args = ["expose", "service", svc, "-n", &ns];
