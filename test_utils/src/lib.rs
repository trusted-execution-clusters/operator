...
impl K8sPlatform for Kind {
    ...
    async fn expose(&self, service: &str, _: &str, _: &str, _: &str) -> Result<()> {
        ...
        let services: Api<Service> = Api::namespaced(self.clients.client.clone(), &self.namespace);
        let service = Service {
            metadata: ObjectMeta {
...
impl K8sPlatform for OpenShift {
    ...
    async fn expose(
        ...
        let watch_services: Api<Service> =
            Api::namespaced(self.clients.watch.clone(), &self.namespace);
        ...
        let certs: Api<Certificate> = Api::namespaced(self.clients.client.clone(), &self.namespace);
        ...
        let deployments: Api<Deployment> =
            Api::namespaced(self.clients.client.clone(), &self.namespace);
...
impl TestContext {
    ...
    async fn create_namespace(&self) -> Result<()> {
        ...
        let namespace_api: Api<Namespace> = Api::all(self.clients.client.clone());
    ...
    {
        let api: Api<K> = Api::namespaced(self.clients.client.clone(), &self.test_namespace);
    ...
    async fn delete_trusted_execution_cluster(&self) -> Result<()> {
        let tec_api: Api<TrustedExecutionCluster> =
            Api::namespaced(self.clients.client.clone(), &self.test_namespace);
    ...
    async fn cleanup_namespace(&self) -> Result<()> {
        let namespace_api: Api<Namespace> = Api::all(self.clients.client.clone());
        let watch_namespaces: Api<Namespace> = Api::all(self.clients.watch.clone());
        let dp = DeleteParams::default();
    ...
    async fn create_certificate(
        ...
        let certs: Api<Certificate> = Api::namespaced(self.clients.client.clone(), ns);
    ...
    async fn set_certificates(&self) -> anyhow::Result<()> {
        ...
        let issuers: Api<Issuer> = Api::namespaced(self.clients.client.clone(), ns);
    ...
    async fn apply_cr_manifests(&self, manifests_path: &Path) -> Result<()> {
        ...
        let depls: Api<Deployment> = Api::namespaced(self.clients.watch.clone(), ns);
        for depl in [
        ...
        let svc = ATTESTATION_KEY_REGISTER_SERVICE;
        let services: Api<Service> = Api::namespaced(self.clients.watch.clone(), ns);
        ...
        let tecs: Api<TrustedExecutionCluster> = Api::namespaced(self.clients.client.clone(), ns);
        let trustee_addr =
        ...
        let configmap_api: Api<ConfigMap> = Api::namespaced(self.clients.client.clone(), ns);
        ...
        test_info!(&self.test_name, "{info}");
        let images: Api<ApprovedImage> = Api::namespaced(self.clients.watch.clone(), ns);
...
async fn setup_test_clients() -> Result<KubeClients> {
    ...
    let watch = Client::try_default().await?;
    Ok(KubeClients { client, watch })
}
