...
impl AzureBackend {
    pub fn new(config: VmConfig) -> Result<Self> {
        let resource_group_prefix = env::var("AZURE_RESOURCE_GROUP_PREFIX").unwrap_or_default();
        let subscription_id = get_env("AZURE_SUBSCRIPTION_ID")?;
        let cred = DeveloperToolsCredential::new(None)?;
        let resource_group = format!("{}{}", resource_group_prefix, config.namespace);
        Ok(Self {
            config,
            client: Client::new(),
...
impl VmBackend for AzureBackend {
    async fn create_vm(&self) -> Result<()> {
        ...
        let rg_url = format!("{mgmt_base}?{NET_API_VERSION}");
        ...
        let nic_url =
            format!("{mgmt_base}/{NET_PATH}/networkInterface/{vm_name}-nic?{NET_API_VERSION}");
        let nic_body = json!({
            "properties": {
        ...
        let shutdown_url = format!(
            "{mgmt_base}/providers/Microsoft.DevTestLab/schedules/shutdown-{vm_name}?{NET_API_VERSION}"
        );
        ...
        self.put_resource(&shutdown_url, &shutdown_body).await?;
        ...
        let vm_body = json!({
            "properties": {
                ...
                "osProfile": {
                    ...
                    "customData": generate_ignition(&self.config),
                },
                "networkProfile": {
                    "networkInterfaces": [
                        {
        ...
        self.put_resource(&vm_url, &vm_body).await.map(|_| ())
    }
    ...
    async fn wait_for_running(&self, timeout_secs: u64) -> Result<()> {
        ...
        let check_fn = || async move {
            ...
            statuses.iter().find(|s| s["code"] == "PowerState/running");
    ...
    async fn ssh_exec(&self, command: &str) -> Result<String> {
        ...
        let full_cmd = format!(
            "ssh -i {} -o IdentitiesOnly=yes -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null {public_ip} -c '{command}'",
    ...
    async fn get_root_key(&self) -> Result<Vec<u8>> {
        let nic_url = format!(
            "{AZURE_MGMT}/{}/{NET_PATH}/networkInterface/{}-nic?{NET_API_VERSION}",
            self.rg_path, self.config.vm_name
        );
