...
const ALLOWED_CRYPTO_CRATES: &[&str] = &[
    ...
    "subtle",
    "zeroize",
    // Pulled in by kbs-client (via kbs_protocol / jwt-simple / cose-rust).
    // kbs-client uses reqwest with native-tls (OpenSSL) for actual TLS.
    // These crates handle JWS/JWE/COSE token processing, not network TLS.
    "aead",
    "aes",
    "aes-gcm",
    "aes-keywrap",
