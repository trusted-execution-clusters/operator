...
// Image component names as they appear in registry paths; no existing constant covers these.
const REGISTRATION_SERVER_COMPONENT: &str = "registration-server";
const COMPUTE_PCRS_COMPONENT: &str = "compute-pcrs";

// Helper functions to get old and current image tags, to define the to and fro upgrade tags.
// UPGRADE_OLD_REGISTRY/UPGRADE_OLD_TAG/UPGRADE_OLD_TRUSTEE_TAG are always set by the Makefile target.
fn old_image(component: &str) -> anyhow::Result<String> {
    let registry = std::env::var("UPGRADE_OLD_REGISTRY")?;
    let tag = std::env::var("UPGRADE_OLD_TAG")?;
    Ok(format!("{registry}/{component}:{tag}"))
}

fn old_trustee_image() -> anyhow::Result<String> {
    let registry = std::env::var("UPGRADE_OLD_REGISTRY")?;
    let tag = std::env::var("UPGRADE_OLD_TRUSTEE_TAG")?;
    Ok(format!("{registry}/key-broker-service:{tag}"))
}
...
// Real version upgrade end-to-end, single VM boot, three phases:
//   Phase 1: install old operator v0.2.2 + Trustee v0.17.0; verify the legacy image-pcrs ConfigMap approach and that a VM attests.
//   Phase 2: upgrade to the current operator + Trustee v0.20.0; verify Trustee is rebuilt from scratch (secret, config, service), ApprovedImage.status.pcrs/events are populated, and the VM re-attests against v0.20.0.
//   Phase 3: a subsequent bad Trustee upgrade must fail (Upgrade=Failed) without disrupting the surviving v0.20.0 Trustee; the VM still attests.
virt_test! {
async fn test_real_version_upgrade() -> anyhow::Result<()> {
    ...
    let old_operator = old_image(OPERATOR_DEPLOYMENT)?;
    let old_trustee = old_trustee_image()?;
    let old_reg_srv = old_image(REGISTRATION_SERVER_COMPONENT)?;
    let old_ak_reg = old_image(ATTESTATION_KEY_REGISTER_DEPLOYMENT)?;
    let old_compute = old_image(COMPUTE_PCRS_COMPONENT)?;
    let image_overrides = [
    ...
    backend.verify_encrypted_root(&root_key).await
        .context("VM should attest with Trustee v0.17.0")?;
    ...
    patch_operator_with_images(
        &deployments,
        &current_image(OPERATOR_DEPLOYMENT),
        &current_trustee_image(),
        &current_image(REGISTRATION_SERVER_COMPONENT),
        &current_image(ATTESTATION_KEY_REGISTER_DEPLOYMENT),
        &current_image(COMPUTE_PCRS_COMPONENT),
    ).await?;
    ...
    assert!(
        deployment_rollout_complete(&trustee_depl),
        "Trustee deployment should be fully rolled out after upgrade"
    );
    ...
    backend.verify_encrypted_root(&root_key).await
        .context("VM should attest against Trustee v0.20.0 after upgrade")?;
    ...
    test_ctx.info(format!("Triggered failing upgrade with bad Trustee image {bad_image}"));
    ...
    let tec = timeout(scaled_duration(120), done)
        .await
        .context("waiting for Upgrade=Failed after bad Trustee upgrade")??
        .expect("TEC should exist after upgrade failure");

    let upgrade_cond = tec.status.as_ref()
        .and_then(|s| s.conditions.as_ref())
        .and_then(|cs| cs.iter().find(|c| c.type_ == UPGRADE_CONDITION))
    ...
    test_ctx.info(format!("Upgrade=Failed detected: {}", upgrade_cond.message));

    // The v0.20.0 Trustee pods must survive the failed upgrade.
    let lp = ListParams::default().labels(&format!("app={TRUSTEE_APP_LABEL}"));
    let surviving = pods_api
        .list(&lp)
        .await?
        .items
        .iter()
        ...
        .any(|_| true);
    assert!(surviving, "Trustee pods should survive the failed upgrade");
    test_ctx.info("Trustee pods survived failed upgrade");

    // Reboot the VM: it must still attest against the surviving v0.20.0 Trustee.
    let boot_id = backend.get_boot_id().await?;
    let _reboot = backend.ssh_exec("sudo systemctl reboot").await;
    backend.wait_for_vm_ssh_ready(scaled_timeout(300), Some(&boot_id)).await?;
    backend.verify_encrypted_root(&root_key).await
        .context("VM should attest against surviving Trustee after failed upgrade")?;
    test_ctx.info("Post-failure attestation verified against surviving Trustee");
